...
| `HomeHeaderView` | Floating OASIS wordmark with the old two counter-rotating logo rings retained behind the private `showsLogoRings` switch but disabled by default. The iOS lockup is intentionally compact (`BrandLockupView` about 0.84×, internal wordmark about 0.82×) for App Store screenshots. While audio is playing, a single async `Canvas` draws the app-icon-inspired halo: continuous cyan/amber ribbons form a slightly tight braided circular ring, every ribbon has its own geometry motion and active-track driver, and active mix nodes add longer softly-faded orbiting glints on top. Keep this as one Canvas surface so the header stays smooth; do not rebuild the ring as many SwiftUI shapes or add colour filters that can make it stutter. The halo's SwiftUI identity follows active source membership, not live slider volume samples, so native slider drags do not restart the decorative timeline. The halo layer fades out softly when playback stops; halo motion and any re-enabled rings freeze under screenshot automation / Reduce Motion. |
| `HomeToolbarImmersiveAudioToggle` | Top-left native toolbar button for the persisted global immersive audio mode. |
| `MixerBoardSectionView` | Mixer list. Inactive rows visually show only the sound name plus quiet controls; active rows can show a compact region label, while the full recording location stays in the accessibility label and detail sheet. Sound and procedural-noise rows share the same rounded glass card chrome with a light horizontal inset so the mixer reads as individual tracks rather than full-width bars. Active row backgrounds stay neutral and translucent instead of adding a channel-tinted wash, so the place photo remains visible behind the controls. Procedural noise rows live at the end of the mixer behind a small `Noise Lab` divider with on/off and volume controls, so ambience-applied white/brown/pink/etc. layers remain manually adjustable after leaving focused listening without interrupting the ambient track order. The list preserves canonical sound order while browsing: toggling a row changes state in place and does not promote active sounds to the top. Active saved ambiences derive rows from the active mix, show only active ambient/noise rows in canonical order, and lock row controls until the ambience is explicitly stopped or replaced. |
| `HapticSlider` | Shared slider behavior for volume controls. The single-value slider stays as SwiftUI's native `Slider` with tint and haptic ticks only, with no extra container. It keeps a view-local draft value during drag and throttles binding/model commits to 24 Hz, with a final commit on release, so `AppModel` persistence/audio sync and decorative header animation do not receive every pointer sample. `AutoVariationRangeSlider` mirrors that bare control style for the two-handle AUTO interval: a full-width neutral rail, tinted selected range, round solid system-like handles that may overhang the rail, haptic ticks, and no outer capsule/glass surface; while dragging, only the active handle becomes interactive Liquid Glass so the rail remains visible through it. Its active handle is gesture-scoped so interrupted drags cannot leave a transparent handle behind, and the solid/glass body swap is not implicitly crossfaded. It keeps a view-local draft range during drag and throttles binding/model commits to 24 Hz, with a final commit on release, so `AppModel` persistence/audio sync does not receive every pointer sample. Only the two handles are draggable, not the track between them. |
...
| `BottomBarView` | Grouped glass playback rail. Secondary actions are visually quiet; playback is the primary centre control. Secondary buttons, the route picker, and the larger playback button share one circle-border style, with only size and active aura changing hierarchy; iOS 26 matched-transition sources on circular side buttons use a `RoundedRectangle` radius equal to half the side-button size, because the API does not accept `Circle()` but the outline must still read as the same circle. The My Ambiences button uses the same `bookmark.fill` SF Symbol as the My Ambiences panel save/library affordance, and highlights only while an ambience/ritual is active, a named noise blend is intact, or the My Ambiences panel is open; leftover procedural noise from a stopped ambience stays editable in the mixer but does not keep the button highlighted. During an active saved ambience, it reduces to playback and audio output only; ambience adjustment moves to the active-status capsule in the toolbar so the listening state does not feel like a full control desk. |
...
Per-channel **tint** is the dominant token: each `SoundChannel` carries an `RGB` tuple in `SoundChannelMetadata.swift`, plus an `OasisGlyph` for the app-owned icon layer. `ChannelMetadata.tint` applies a central HSB saturation/brightness boost for a vibrant but non-neon runtime colour, and that tint flows into:

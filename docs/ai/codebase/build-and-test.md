...
- `testLockedSavedAmbienceShowsInlineUpsellBeforePaywall`
- `testSavingAmbienceRequiresPremiumForFreeUsers`
...
- `testSavedAmbienceStartsLocalizedMixAndKeepsMixerClean`
- `testSavedAmbienceCanBeReplacedInTwoTaps`

My Ambiences, shuffle, and binaural entry points live in the bottom bar, so the tests target `home.bottom.compose`, `home.bottom.shuffle`, and `home.bottom.binaural` (not the older header identifiers). The My Ambiences tests launch without `-ui_testing` so the first bottom-bar slot remains `home.bottom.compose` instead of the screenshot-automation shuffle compatibility button. They currently verify the full-screen My Ambiences empty state, top save action `compose.ambience.save`, disabled export action `compose.ambience.export`, and free-user save gating via `premium.inline.preset`. Reintroduce saved-ambience playback/replace assertions when the iPhone-authored default presets are imported. Run these manually or in Xcode when touching premium gating, My Ambiences, or the bottom bar; the App Store screenshot lane intentionally filters them out.

## iPhone-authored default ambience import

Create candidate ambiences on a real iPhone, then export them from My Ambiences with `compose.ambience.export`. Save the JSON into the repo and run:

```bash
ruby scripts/exported_presets_to_swift.rb path/to/oasis-iphone-ambiences.json
```

The script prints a Swift `return [` body for `Array.defaultPresets()` and fails unless exactly two exported ambiences are free-access. Additional defaults should be premium by content: premium channel, premium noise, premium binaural track, or timer above 30 minutes.

...
- When a decorative trail must follow a curved or breathing path, prefer a small `Canvas` that samples past positions on the real trajectory instead of layering fixed `Shape` strokes with a simple gradient; use enough samples and connected strokes for a continuous ribbon, not bead-like particles.
...
- Keep custom range sliders visually aligned with SwiftUI's native `Slider`: no extra outer container, a full-width neutral rail, tinted selected range, solid system-like handles that can overhang the rail edges, haptic ticks, and explicit VoiceOver actions. If mimicking the iOS 26 native slider interaction, apply interactive Liquid Glass only to the actively dragged handle so the rail remains visible through it; avoid always-on glass handles, which visually merge into blobs instead of reading as native controls. Keep the active handle in gesture-scoped state rather than persistent `@State`, and do not implicitly animate the solid/glass body swap; otherwise interrupted drags can leave glass ghosts or transparent handles. During drag, keep pointer-sample updates in local view state and throttle binding/model commits below the 32 Hz system reporting threshold (currently 24 Hz), then force one final commit on release.
